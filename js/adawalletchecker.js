...
/* fetch("https://pool.pm/stake/4d4e74fd118c8ac009b8c9fcb7e5e8d6ef297bbdc36aa794282c71ef")
.then((response) => response.json())
.then((data) => {
	console.log("AGEEEEEEEE:", data['stake1uy4vgtanyj6xdkqp32j49vayhakde5uxe8rpc8rsjl3a63qx3w6z0'])
}); */

/* addr1q9csysy0d0fev0tcr2ss8vjnqy2hcm4ua4yhy23lk5z7jv74vx0xf62lrv0gh5csahdz2gt9k09vnn86cl9fhllp5sjs2m2gnc */

document.getElementById('adaTokensTable').style.display = 'none';
document.getElementById('adaNFTTable').style.display = 'none';
document.getElementById('noTokensAndNFTS').style.display = 'none';
document.getElementById('stakeReward').style.display = 'none';
document.getElementById('ifStaked').style.display = 'none';
document.getElementById('cardanoBalance').style.display = 'none';

async function getWalletInfo() {
  resetInfo();
	jsondata = await getJson(apiUrl)
  adaAddress = document.getElementById('adaAddress').value;
  console.log(adaAddress)
  fetch("https://pool.pm/wallet/" + adaAddress)
	.then(function(response){
    if(!response.ok){
      document.getElementById('alerText').innerHTML = "დაფიქსირდა შეცდომა! ცადეთ მოგვიანებით.";
      document.getElementById('noTokensAndNFTS').style.display = 'block';
    }
    return response.json()
  })
	.then((data) => {
    document.getElementById('stakeReward').style.display = 'block';
    document.getElementById('ifStaked').style.display = 'block';
    document.getElementById('cardanoBalance').style.display = 'block';
    console.log(data);
		document.getElementById('adaBalance').innerHTML = `${(data.lovelaces / 1000000).toLocaleString("en-US")}`
		document.getElementById('pendingReward').innerHTML = `${(data.reward / 1000000).toLocaleString("en-US")}`
		
		if (data.pool){
			let poolID = data.pool;
			document.getElementById('isStaked').innerHTML = `<a href="${jsondata[poolID].homepage}">${jsondata[poolID].ticker}</a>`;
		}else{
			document.getElementById('isStaked').innerHTML = `არა`;
		}

    if(data.tokens.length > 0){
      for (let i = 0; i < data.tokens.length; i++) {
        if (data.tokens[i].metadata.image) {
          document.getElementById('adaNFTTable').style.display = 'block';
          let content = ` 
          <div class="col-sm-2 mb-2">
            <div class="card card-profile card-plain">
              <div class="card-body text-center bg-white shadow border-radius-lg p-3">
                <a target="_blank" href="https://ipfs.io/ipfs/${(data.tokens[i].metadata.image).replace('ipfs://','')}">
                  <img class="w-75 border-radius-md" src="https://ipfs.io/ipfs/${(data.tokens[i].metadata.image).replace('ipfs://','')}">
                </a>
                <h6 class="mt-3 mb-1 d-md-block d-none">${data.tokens[i].name}</h6>
                <p class="mb-1 d-md-none d-block text-sm font-weight-bold text-darker">${data.tokens[i].name}</p>
                <p class="mb-0 text-xs font-weight-bolder text-warning text-gradient text-uppercase">${data.tokens[i].quantity}</p>
              </div>
            </div>
          </div>`;
            adaNFTBalance.innerHTML += content;
        }else{
          document.getElementById('adaTokensTable').style.display = 'block';
          let content = `
            <tr>
                <td>
                <div class="d-flex px-2 py-1">
                    <div class="d-flex flex-column justify-content-center">
                    <h6 class="mb-0 text-sm">${data.tokens[i].name}</h6>
                    <p class="text-xs text-secondary mb-0"></p>
                    </div>
                </div>
                </td>
                <td class="align-middle">
                <p class="text-xs font-weight-bold mb-0">${(data.tokens[i].quantity.toLocaleString("en-US"))}</p>
                </td>
                <td class="align-middle text-center">
                <p class="text-xs font-weight-bold mb-0">${shortAdaAddress(data.tokens[i].policy)}</p>
                </td>
                                    <td class="align-middle text-center">
                <p class="text-xs font-weight-bold mb-0">${shortAdaAddress(data.tokens[i].fingerprint)}</p>
                </td>
            </tr>`;
            adaTokenBalance.innerHTML += content;
        }
      }
    }else{
      document.getElementById('alerText').innerHTML = "ამ მისამართზე ტოკენები და NFT არ გაქვთ!";
      document.getElementById('noTokensAndNFTS').style.display = 'block';
    }
	});
}
...
function resetInfo(){
  ...
  document.getElementById('ifStaked').style.display = 'none';
  document.getElementById('cardanoBalance').style.display = 'none';
  document.getElementById('adaTokenBalance').innerHTML = "";
  document.getElementById('adaNFTBalance').innerHTML = "";
