...
document.getElementById('pendingMeld').style.display = 'none';

async function meldISPO(){
    var pool1 = 'https://stakepool-worker.meld-labs.workers.dev/active-stakes?&pool_id=0&fromEpoch=0&toEpoch=1000';
    var pool2 = 'https://stakepool-worker.meld-labs.workers.dev/active-stakes?&pool_id=1&fromEpoch=0&toEpoch=1000';
    var pool3 = 'https://stakepool-worker.meld-labs.workers.dev/active-stakes?&pool_id=2&fromEpoch=0&toEpoch=1000';
    var pool5 = 'https://stakepool-worker.meld-labs.workers.dev/active-stakes?&pool_id=3&fromEpoch=0&toEpoch=1000';
    var pool6 = 'https://stakepool-worker.meld-labs.workers.dev/active-stakes?&pool_id=4&fromEpoch=0&toEpoch=1000';
    var pool7 = 'https://stakepool-worker.meld-labs.workers.dev/active-stakes?&pool_id=5&fromEpoch=0&toEpoch=1000';
    var pool8 = 'https://stakepool-worker.meld-labs.workers.dev/active-stakes?&pool_id=6&fromEpoch=0&toEpoch=1000';
    var pool9 = 'https://stakepool-worker.meld-labs.workers.dev/active-stakes?&pool_id=7&fromEpoch=0&toEpoch=1000';
    var poolX = 'https://stakepool-worker.meld-labs.workers.dev/active-stakes?&pool_id=9&fromEpoch=0&toEpoch=1000';

    var selectMeldPool = document.getElementById('meldPoolSelection');
    var optionMeldPool = selectMeldPool.options[selectMeldPool.selectedIndex];

    let stakedAddress = document.getElementById('stake1MeldAddress').value;

    if (optionMeldPool.value == "meldpool9"){
        fetch(pool9)
        .then(function(response){
            if(!response.ok){
                document.getElementById('pendingMeld').style.display = 'block';
                document.getElementById('pendingMeld').innerHTML = "დაფიქსირდა შეცდომა!";
            }
            return response.json()
        })
        .then(data => {
            console.log(data.main[stakedAddress])
            if (data.main[stakedAddress]){
                document.getElementById('pendingMeld').style.display = 'block';
                document.getElementById('pendingMeld').innerHTML = "დაგროვებული: " + (data.main[stakedAddress].reward  / 1000000).toFixed(2) + " MELD";
            }else{
                document.getElementById('pendingMeld').style.display = 'block';
                document.getElementById('pendingMeld').innerHTML = "ამ Pool-ში დასტეიკებული არ გაქვს";
            }
        })
    }else if(optionMeldPool.value == "meldpool1"){
        fetch(pool1)
        .then(function(response){
            if(!response.ok){
                document.getElementById('pendingMeld').style.display = 'block';
    ...
    }else if(optionMeldPool.value == "meldpool2"){
        fetch(pool2)
        .then(function(response){
            if(!response.ok){
                document.getElementById('pendingMeld').style.display = 'block';
    ...
    }else if(optionMeldPool.value == "meldpool3"){
        fetch(pool3)
        .then(function(response){
            if(!response.ok){
                document.getElementById('pendingMeld').style.display = 'block';
    ...
    }else if(optionMeldPool.value == "meldpool5"){
        fetch(pool5)
        .then(function(response){
            if(!response.ok){
                document.getElementById('pendingMeld').style.display = 'block';
    ...
    }else if(optionMeldPool.value == "meldpool6"){
        fetch(pool6)
        .then(function(response){
            if(!response.ok){
                document.getElementById('pendingMeld').style.display = 'block';
    ...
    }else if(optionMeldPool.value == "meldpool7"){
        fetch(pool7)
        .then(function(response){
            if(!response.ok){
                document.getElementById('pendingMeld').style.display = 'block';
    ...
    }else if(optionMeldPool.value == "meldpool8"){
        fetch(pool8)
        .then(function(response){
            if(!response.ok){
                document.getElementById('pendingMeld').style.display = 'block';
    ...
    }else if(optionMeldPool.value == "meldpool9"){
        fetch(pool9)
        .then(function(response){
            if(!response.ok){
                document.getElementById('pendingMeld').style.display = 'block';
    ...
    }else if(optionMeldPool.value == "meldpoolX"){
        fetch(poolX)
        .then(function(response){
            if(!response.ok){
                document.getElementById('pendingMeld').style.display = 'block';
        ...
        document.getElementById('pendingMeld').style.display = 'block';
        document.getElementById('pendingMeld').innerHTML = "გთხოვთ აირჩიოთ Pool!";
    }

}

document.getElementById('minswapStakingTable').style.display = 'none';
document.getElementById('pendingMIN').style.display = 'none';
document.getElementById('pendingMINt').style.display = 'none';

async function minSwapStaking(){
    document.getElementById('minswapStakingTable').style.display = 'none';
    document.getElementById('pendingMIN').style.display = 'none';
    document.getElementById('pendingMINt').style.display = 'none';
    let stakedAddress = document.getElementById('stake1MinSwapAddress').value;
    let pendingMIN = [];
    let pendingMINt = [];
    fetch('https://api-mainnet.minswap.org/fiso/rewards/' + stakedAddress)
    .then(function(response){
        if(!response.ok){
            document.getElementById('pendingMIN').style.display = 'block';
            document.getElementById('pendingMIN').innerHTML = "დაფიქსირდა შეცდომა!"
        }
        return response.json()
    })
    .then(data => {
        console.log(data);
        if(data != null){
            document.getElementById('minswapStakingTable').style.display = 'block';
            data.forEach(t =>{
                pendingMIN.push(t.amountMin / 1000000);
                pendingMINt.push(t.amountMint / 1000000);
                let content = `
                <tr>
                    <td class="align-middle text-center">
                        <p class="text-sm font-weight-bold mb-0">${t.epoch}</p>
                    </td>
                    <td class="align-middle">
                        <p class="text-sm font-weight-bold mb-0">${t.poolName}</p>
                    </td>
                    <td class="align-middle text-center">
                        <p class="text-sm font-weight-bold mb-0">${(t.amountDelegate / 1000000).toFixed(2)}</p>
                    </td>
                    <td class="align-middle text-center">
                        <p class="text-sm font-weight-bold mb-0">${(t.amountMin / 1000000).toFixed(2)}</p>
                    </td>
                    <td class="align-middle text-center">
                        <p class="text-sm font-weight-bold mb-0">${(t.amountMint / 1000000).toFixed(2)}</p>
                    </td>
                </tr>
                `
                minswapPendingRewards.innerHTML += content;
            })
    
            var totalPendingMIN = pendingMIN.reduce(function(a, b) { return a + b; }, 0);
            var totalPendingMINt = pendingMINt.reduce(function(a, b) { return a + b; }, 0);
    
            let content = `
            <tr>
                <td class="align-middle text-center">
                    <p style="color: black;" class="text-md font-weight-bold mb-0">${data.length} (${data.length * 5} დღე)</p>
                </td>
                <td class="align-middle">
                ...
                <td class="align-middle text-center">
                    <p style="color: black;" class="text-md font-weight-bold mb-0">${(totalPendingMIN).toFixed(2)}</p>
                </td>
                <td class="align-middle text-center">
                    <p style="color: black;" class="text-md font-weight-bold mb-0">${(totalPendingMINt).toFixed(2)}</p>
                </td>
            </tr>
            `
            minswapPendingRewards.innerHTML += content;  
            document.getElementById('pendingMIN').style.display = 'block';
            document.getElementById('pendingMINt').style.display = 'block'; 
            document.getElementById('pendingMIN').innerHTML = `დაგორვებული MIN: ${(totalPendingMIN).toFixed(2)}`;
            document.getElementById('pendingMINt').innerHTML = `დაგორვებული MINt: ${(totalPendingMINt).toFixed(2)}`;
        }else{
            document.getElementById('pendingMIN').style.display = 'block';
            document.getElementById('pendingMIN').innerHTML = "დაფიქსირდა შეცდომა!"
        }
