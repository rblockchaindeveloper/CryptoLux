...
function exchangesList(){
    ...
    /* ბაზრის მარკეტკაპი, ნავაჭრი და სულ კრიპტოვალუტები */
    ...
    .then(data => {
        ...
        if (data.data.market_cap_change_percentage_24h_usd > 0){
            var mcapChangePercent = `<span class="text-success text-xs font-weight-bolder"> (${(data.data.market_cap_change_percentage_24h_usd).toFixed(2)}%)</span>`;
        }else{
            var mcapChangePercent = `<span class="text-danger text-xs font-weight-bolder"> (${(data.data.market_cap_change_percentage_24h_usd).toFixed(2)}%)</span>`;
        }

        document.getElementById('totalMarketCap').innerHTML = `$${Math.floor(data.data.total_market_cap.usd).toLocaleString("en-US")} ${mcapChangePercent}`;
        document.getElementById('total24hrVolume').innerHTML = "$" + Math.floor(data.data.total_volume.usd).toLocaleString("en-US");
        document.getElementById('totalCryptoCurrencies').innerHTML = (data.data.active_cryptocurrencies).toLocaleString("en-US");

        

        /* კრიპტოების დომინირების პროცენტი */
        fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=10&page=1&sparkline=false&price_change_percentage=1h%2C24h%2C7d')
        .then(function(response){
            if(!response.ok){
                console.log("THERE IS ERROR SHECH")
            }
            return response.json()
        })
        .then(dataP => {
            
            for (let i = 0; i < Object.values(data.data.market_cap_percentage).length; i++) {
                var coinPercent = Object.values(data.data.market_cap_percentage)[i];
                let content = `
                <tr>
                    <td>
                    <div class="d-flex px-2 py-1">
                        <div class="icon icon-shape icon border-radius-sm text-center me-2 d-flex align-items-center justify-content-center">
                        <img src="${dataP[i].image}" style="width: 30px;" alt="">
                        </div>
                        <div class="d-flex flex-column justify-content-center">
                        <h6 class="mb-0 text-sm">${(dataP[i].symbol).toUpperCase()} დომინირებს ${(coinPercent).toFixed(2)}%-ით
                        </h6>
                        </div>
                    </div>
                    </td>
                ...
                `
                coinDominances.innerHTML += content
            }
        })
        /* კრიპტოების დომინირების პროცენტი */
    ...
    .then(data => {
        ...
        data.forEach(coin =>{
            if (coin.price_change_percentage_24h > 0){
                var priceChange24Percent = `<p style="color: green;">${(coin.price_change_percentage_24h).toFixed(2)}%</p>`
            }else{
                var priceChange24Percent = `<p style="color: red;">${(coin.price_change_percentage_24h).toFixed(2)}%</p>`
            }

            if (coin.price_change_percentage_1h_in_currency > 0){
                var priceChange1Percent = `<p style="color: green;">${(coin.price_change_percentage_1h_in_currency).toFixed(2)}%</p>`
                if (coin.current_price < 0.001){
                    var currentPrice = `<span style="color: green;">$${(coin.current_price).toFixed(8)}</span>`
                }else{
                    var currentPrice = `<span style="color: green;">$${(coin.current_price).toLocaleString('en-US')}</span>`
                }
            }else{
                var currentPrice = `<span style="color: red;">$${(coin.current_price).toLocaleString('en-US')}</span>`
                var priceChange1Percent = `<p style="color: red;">${(coin.price_change_percentage_1h_in_currency).toFixed(2)}%</p>`
            }

            if (coin.price_change_percentage_7d_in_currency > 0){
                var priceChange7Percent = `<p style="color: green;">${(coin.price_change_percentage_7d_in_currency).toFixed(2)}%</p>`
            }else{
                var priceChange7Percent = `<p style="color: red;">${(coin.price_change_percentage_7d_in_currency).toFixed(2)}%</p>`
            }

            if (coin.market_cap_change_percentage_24h > 0){
                var mCapPercent = `<p style="color: green;">${(coin.market_cap_change_percentage_24h).toFixed(2)}%</p>`
            }else{
                var mCapPercent = `<p style="color: red;">${(coin.market_cap_change_percentage_24h).toFixed(2)}%</p>`
            }

            let content = `
            <tr>
                <td class="align-middle text-center">
                <p class="text-sm font-weight-bold mb-0">${coin.market_cap_rank}</p>
                </td>
                <td>
                <div class="d-flex px-2 py-1">
                    <div class="icon icon-shape icon-sm text-center border-radius-lg me-2">
                    <img src="${coin.image}" style="width: 25px;" class="mt-1" alt="">
                    </div>
                    <div class="d-flex flex-column justify-content-center">
                    <h6 class="mb-0 text-sm"> <a target="_blank" href="https://www.coingecko.com/en/coins/${coin.id}">${coin.name}</a>
                    </h6>
                    </div>
                </div>
                </td>
                <td class="align-middle">
                <p class="text-sm font-weight-bold mb-0">${currentPrice}</p>
                </td>
                <td class="align-middle text-center">
                <p class="text-sm font-weight-bold">${priceChange1Percent}</p>
                </td>
                <td class="align-middle text-center">
                <p class="text-sm font-weight-bold">${priceChange24Percent}</p>
                </td>
                <td class="align-middle text-center">
                <p class="text-sm font-weight-bold mb-0">$${formatCash(coin.total_volume)}</p>
                </td>
                <td class="align-middle text-center">
                <p class="text-sm font-weight-bold mb-0">$${formatCash(coin.market_cap)}</p>
                </td>
            </tr>
            `

            top10Crypto.innerHTML += content;
        }) 
        
    })
    /* ტოპ 10 კრიპტოვალუტა */

    /* ბიტკოინის გრიდ ანდ ფეარ ინდეხ */

    fetch('https://api.alternative.me/fng/?limit=40')
    .then(function(response){
        if(!response.ok){
            console.log("THERE IS ERROR SHECH")
        }
    ...
    .then(data => {
        ...
        if (data.data[today].value > 60){
            var indNumber = `bg-gradient-success`;
        }else if(45 < data.data[today].value < 55){
            var indNumber = `bg-gradient-warning`;
        }else if(data.data[today].value < 40){
            var indNumber = `bg-gradient-danger`;
        }

        if (data.data[today].value_classification == 'Greed'){
            var indFeel = 'სიხარბე'
        }else if(data.data[today].value_classification == 'Extreme Greed'){
            var indFeel = 'დიდი სიხარბე'
        }else if(data.data[today].value_classification == 'Neutral'){
            var indFeel = 'ნეიტრალური'
        }else if(data.data[today].value_classification == 'Fear'){
            var indFeel = 'შიში'
        }else if(data.data[today].value_classification == 'Extreme Fear'){
            var indFeel = 'დიდი შიში'
        }
        ...
        <tr>
            <td>
            <div class="d-flex px-2 py-1">
                <div class="icon icon-shape icon-sm shadow border-radius-sm ${indNumber} text-center me-2 d-flex align-items-center justify-content-center">
                <span class="text-white">${data.data[today].value}</span>
                </div>
                <div class="d-flex flex-column justify-content-center">
                <h6 id="ocBSC" class="mb-0 text-sm">დღეს არის ${indFeel}
                </h6>
                </div>
            </div>
            </td>
        ...
        fgIndexTable.innerHTML += info;

        /* =================================== */

        if (data.data[yesterday].value > 60){
            var indNumber = `bg-gradient-success`;
        }else if(45 < data.data[yesterday].value < 55){
            var indNumber = `bg-gradient-warning`;
        }else if(data.data[yesterday].value < 40){
            var indNumber = `bg-gradient-danger`;
        }

        if (data.data[yesterday].value_classification == 'Greed'){
            var indFeel = 'სიხარბე'
        }else if(data.data[yesterday].value_classification == 'Extreme Greed'){
            var indFeel = 'დიდი სიხარბე'
        }else if(data.data[yesterday].value_classification == 'Neutral'){
            var indFeel = 'ნეიტრალური'
        }else if(data.data[yesterday].value_classification == 'Fear'){
            var indFeel = 'შიში'
        }else if(data.data[yesterday].value_classification == 'Extreme Fear'){
            var indFeel = 'დიდი შიში'
        }
        ...
        <tr>
            ...
            <div class="d-flex px-2 py-1">
                <div class="icon icon-shape icon-sm shadow border-radius-sm ${indNumber} text-center me-2 d-flex align-items-center justify-content-center">
                <span class="text-white">${data.data[yesterday].value}</span>
                </div>
                <div class="d-flex flex-column justify-content-center">
                <h6 id="ocBSC" class="mb-0 text-sm">გუშინ იყო ${indFeel}
                </h6>
                </div>
            </div>
            </td>
        ...
        `
        fgIndexTable.innerHTML += info2;

        /* =================================== */

        if (data.data[lastweek].value > 60){
            var indNumber = `bg-gradient-success`;
        }else if(45 < data.data[lastweek].value < 55){
            var indNumber = `bg-gradient-warning`;
        }else if(data.data[lastweek].value < 40){
            var indNumber = `bg-gradient-danger`;
        }

        if (data.data[lastweek].value_classification == 'Greed'){
            var indFeel = 'სიხარბე'
        }else if(data.data[lastweek].value_classification == 'Extreme Greed'){
            var indFeel = 'დიდი სიხარბე'
        }else if(data.data[lastweek].value_classification == 'Neutral'){
            var indFeel = 'ნეიტრალური'
        }else if(data.data[lastweek].value_classification == 'Fear'){
            var indFeel = 'შიში'
        }else if(data.data[lastweek].value_classification == 'Extreme Fear'){
            var indFeel = 'დიდი შიში'
        }
        ...
        <tr>
            ...
            <div class="d-flex px-2 py-1">
                <div class="icon icon-shape icon-sm shadow border-radius-sm ${indNumber} text-center me-2 d-flex align-items-center justify-content-center">
                <span class="text-white">${data.data[lastweek].value}</span>
                </div>
                <div class="d-flex flex-column justify-content-center">
                <h6 id="ocBSC" class="mb-0 text-sm">წინა კვირას იყო ${indFeel}
                </h6>
                </div>
            </div>
            </td>
        ...
        `
        fgIndexTable.innerHTML += info3;

        /* =================================== */

        if (data.data[lastMonth].value > 60){
            var indNumber = `bg-gradient-success`;
        }else if(45 < data.data[lastMonth].value < 50){
            var indNumber = `bg-gradient-warning`;
        }else if(data.data[lastMonth].value < 40){
            var indNumber = `bg-gradient-danger`;
        }

        if (data.data[lastMonth].value_classification == 'Greed'){
            var indFeel = 'სიხარბე'
        }else if(data.data[lastMonth].value_classification == 'Extreme Greed'){
            var indFeel = 'დიდი სიხარბე'
        }else if(data.data[lastMonth].value_classification == 'Neutral'){
            var indFeel = 'ნეიტრალური'
        }else if(data.data[lastMonth].value_classification == 'Fear'){
            var indFeel = 'შიში'
        }else if(data.data[lastMonth].value_classification == 'Extreme Fear'){
            var indFeel = 'დიდი შიში'
        }
        ...
        <tr>
            ...
            <div class="d-flex px-2 py-1">
                <div class="icon icon-shape icon-sm shadow border-radius-sm ${indNumber} text-center me-2 d-flex align-items-center justify-content-center">
                <span class="text-white">${data.data[lastMonth].value}</span>
                </div>
                <div class="d-flex flex-column justify-content-center">
                <h6 id="ocBSC" class="mb-0 text-sm">წინა თვეში იყო ${indFeel}
                </h6>
                </div>
            </div>
            </td>
    ...
    .then(data => {
        document.getElementById('defiMarketCap').innerHTML = "კაპიტალიზაცია - $" + formatCash(data.data.defi_market_cap);
        document.getElementById('ethMarketCap').innerHTML = "ETH კაპიტალიზაცია - $" + formatCash(data.data.eth_market_cap);
        document.getElementById('defiVolume24h').innerHTML = "ნავაჭრი 24სთ - $" + formatCash(data.data.trading_volume_24h);
        document.getElementById('defiDominance').innerHTML = "DeFi დომინირებს - " + parseFloat(data.data.defi_dominance).toFixed(2) + "%-ით";
        document.getElementById('defiTopCoin').innerHTML = "DeFi-ს TOP კირპტო - " + data.data.top_coin_name;
        document.getElementById('defiTopCoinDominance').innerHTML = "TOP კრიპტო დომინირებს - " + parseFloat(data.data.top_coin_defi_dominance).toFixed(2) + "%-ით";
    })
