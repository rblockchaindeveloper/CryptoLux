...
/* ქსელების TVL იწყება აქ  */
async function pCounter(chainName){
    var chainProtocols = [];
    let data = await fetch('https://api.llama.fi/protocols')
    .then(response => response.json())
    .then(data => {
        return data;
    })
    ...
    data.forEach(p =>{
        if(p.chains.includes(chainName)){
            chainProtocols.push(p.name);
        }
    })
    return chainProtocols.length;
}

var chains = [];
getTVLs();
async function getTVLs(){
    let data = await fetch('https://api.llama.fi/chains')
    .then(response => response.json())
    .then(data => {
        const chainTVLList = document.getElementById('chainTVLList');
        return data;
    })
    .catch(error => {
        console.error(error);
    ...
    for (let i = 0; i < chains[0].length; i++){
        var dappNumber = await pCounter(chains[0][i].name);
        Object.assign(chains[0][i], {dapps: dappNumber});
    }

    chains[0].sort(function(a, b) {
        var keyA = (a.tvl), keyB = (b.tvl);
        ...
        let content = `
            <tr>
                ...
                <div class="d-flex px-2 py-1">
                    <div class="me-3">
                    <img src="https://icons.llama.fi/chains/rsz_${(chains[0][i].name).toLowerCase()}.jpg" style="width: 30px;" class="mt-1 rounded-circle" alt="">
                    </div>
                    <div class="d-flex alig flex-column justify-content-center">
                    <h6 class="mb-0 text-sm"><a target="_blank" href="https://defillama.com/chain/${chains[0][i].name}">${chains[0][i].name}</a></h6>
                    </div>
                </div>
                </td>
                <td class="align-middle ">
                <p class="text-xs font-weight-bold mb-0">${chains[0][i].dapps}</p>
                </td>
                <td class="align-middle ">
                <p class="text-xs font-weight-bold mb-0"><a target="_blank" href="https://www.coingecko.com/en/coins/${chains[0][i].gecko_id}">${chains[0][i].tokenSymbol ? chains[0][i].tokenSymbol : errorIcon}</a></p>
                </td>
                <td class="align-middle">
                <p class="font-weight-bold mb-0"><a href="#">$${formatCash(chains[0][i].tvl)}</a></p>
                </td>
            </tr>
            `
        chainTVLList.innerHTML += content;
        counter += 1;
    }
}
/* ქსელების TVL იწყება აქ  */
...
getDeFiChart();
async function getDeFiChart() {
    let liqData = [];
    let liqDate = [];
    let data = await fetch('https://api.llama.fi/charts')
    .then(response => response.json())
    .then(data => {
        document.getElementById('tvlTitle').innerHTML = "TVL სულ: $" + formatCash(data[data.length - 1].totalLiquidityUSD);
        var months = ['იან', 'თებ', 'მარ', 'აპრ', 'მაი', 'ინვ', 'ივლ', 'აგვ', 'სექ', 'ოქტ', 'ნოე', 'დეკ'];
        for (let i = 0; i < data.length; i++){
            var a = new Date(data[i].date * 1000);
            var month = months[a.getMonth()];
            var year = a.getFullYear();
            var date = a.getDate();
            var time = date + ' ' + month + ' ' + year;
            liqData.push(data[i].totalLiquidityUSD)
            liqDate.push(time)
        }
