...
function renderNFTTransfer(nftName, nftAddress, nftID) {
    ...
    <div class="modal-footer mt-2">
        <button type="button" class="btn btn-gradient-secondary" data-bs-dismiss="modal">დახურვა</button>
        <button onclick="tryNFTTransfer('${nftAddress}', '${nftID}', '${nftName}')" data-bs-dismiss="modal" type="button" class="btn bg-gradient-primary"}">გაგზავნა</button>
    </div>
    `);
}
...
async function tryNFTTransfer(nftAddress, nftID, nftName) {
    ...
    if (address == ""){
        ...
        document.getElementById('confirmNftInfoHash').innerHTML = `
        <p class="text-lg mt-1 mb-1 mb-3 font-weight-bolder text-center" style="color: #290c5f;">${nftName}</p>
        ...
        ` 
        $("#nftConfirmModal").modal('show');

        let confirmTxbtn = document.getElementById('confirmNftSend');

        confirmTxbtn.onclick = async function(){
            $("#nftConfirmModal").modal('hide');
...
async function nftTransfer(address, tokenContract, nftID) {
    ...
    try{
        ...
        result.on("transactionHash", (hash) => { 
                document.getElementById('sendTxNFTInfoHash').innerHTML = 
                `
