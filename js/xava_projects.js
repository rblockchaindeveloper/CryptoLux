...
/* აუთენთიფიკაციის ფუნქციები */
getBscPadProjects();
function getBscPadProjects() {
  try {
    fetch("https://avalaunch-kyc.herokuapp.com/api/v1/projects")
      .then(function (response) {
        if (!response.ok) {
          let content = `
                <div class="alert alert-danger" role="alert">
                    <strong>NOTOK!</strong> დაფიქსირდა შეცდომა!
                </div>
                `;

          closed_projects.innerHTML = content;
        }
      ...
      .then((data) => {
        ...
        for (let i = 0; i < data.projects.length; i++) {
          console.log(data.projects[i]);
			 ...
			 if (data.projects[i].current_round == "Sale ended."){

			 }

          let content = `
					<div class="col-lg-12 col-xl-4 mt-xl-0 mt-4 mb-4">
						<div class="card h-60">
							...
							<div class="card-body p-3">
									<ul class="list-group">
										<li class="list-group-item border-0 d-flex align-items-center px-0 mb-2">
											<div class="me-2">
													<img src="${data.projects[i].project_logo}" style="width: 40px;" alt="">
											</div>
											<div class="d-flex align-items-start flex-column justify-content-center">
													<h6 class="mb-0 text-sm">${data.projects[i].title}</h6>
													<div class="d-flex">
														<a target="_blank" href="${data.projects[i].website_url}"><p class="mb-0 text-sm me-1" style="cursor: pointer;"><i class="fas fa-globe"></i></p></a>
														<a target="_blank" href="${data.projects[i].socials.twitter}"><p class="mb-0 text-sm me-1" style="cursor: pointer;"><i class="fab fa-twitter"></i></p></a>
														<a target="_blank" href="${data.projects[i].socials.telegram}"><p class="mb-0 text-sm me-1" style="cursor: pointer;"><i class="fab fa-telegram"></i></p></a>
													</div>
											</div>
										</li>
									</ul>
									...
									<label class="label mb-0">ფასი: </label>
									<div class="text-sm text-bold text-dark"> 1 ${data.projects[i].token.symbol} = $${data.projects[i].token_price_in_usd}</div>
			 ...
			 if (data.projects[i].current_round == "Sale ended."){
				...
				opened_projects.innerHTML += content;
			 }
