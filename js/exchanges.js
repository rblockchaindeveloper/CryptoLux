...
function exchangesList(){
    fetch('https://api.coingecko.com/api/v3/exchanges?per_page=250')
    .then(response => response.json())
    .then(data => {
        console.log(data);
        ...
        data.forEach(ex =>{
            ...
            <tr>
                ...
                <div class="d-flex px-2 py-1">
                    ...
                    <img src="${ex.image}" style="width: 25px;" class="mt-1" alt="">
                    </div>
                    <div class="d-flex flex-column justify-content-center">
                    <h6 class="mb-0 text-sm"> <a target="_blank" href="${ex.url}">${ex.name}</a></h6>
                    </div>
                </div>
                </td>
                <td class="align-middle text-center">
                <p class="text-sm font-weight-bold">${ex.year_established ? ex.year_established : errorIcon}</p>
                </td>
                <td class="align-middle text-center">
                <p class="text-sm font-weight-bold">${ex.country ? ex.country : errorIcon}</p>
                </td>
                <td class="align-middle text-center">
                <p class="text-sm font-weight-bold">${ex.trust_score ? ex.trust_score : errorIcon}</p>
                </td>
                <td class="align-middle text-center">
                <p class="text-sm font-weight-bold">${(ex.trade_volume_24h_btc).toLocaleString("en-US")} BTC</p>
                </td>
            </tr>
            `
            exchangeList.innerHTML += content;
            counter += 1;
        })
    })
}
